...
export async function transport<T>(
  ...
  try {
    ...
    throw new ApiError(
      caught instanceof Error ? caught.message : "The spend-controls gateway could not be reached.",
      0,
      "network_error",
    );
  }
...
export function apiErrorMessage(caught: unknown): string {
  ...
  if (caught.status === 0) return `Unable to reach the spend-controls gateway. Check your connection and try again.${suffix}`;
