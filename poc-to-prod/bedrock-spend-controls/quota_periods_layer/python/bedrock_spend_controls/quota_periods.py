...
entry is an *alert-only* budget that warns but never blocks. Rows written
before thresholds existed resolve to the deployment default
``[{warn_threshold}, {100 % block}]``, so behaviour is unchanged until an
operator configures a list.
...
# tokens so token quotas keep their pre-caching meaning).
LEDGER_COUNTERS = (
    "cost_micro",
    "input_tokens",
    "output_tokens",
...
class QuotaBreach:
    ...
    # Utilization at which this period blocks (basis points); 10_000 for
    # legacy rows. Present so notifications can say "blocked at 150 %".
    at_bps: int = BPS
...
def default_thresholds(
    ...
    """The pre-thresholds behaviour: one warning, then block at 100 %."""
    return [
...
def limit_attributes(
    ...
    *,
    ...
    so every row written through this path is self-describing.
    """
...
def limits_from_item(
    ...
    *,
    ...
    Rows written before thresholds existed have no ``{period}_thresholds``
    attribute and resolve to ``default_thresholds_list`` (the deployment
