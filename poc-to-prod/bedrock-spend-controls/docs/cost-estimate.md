...
| Permission lease (runtime dial) | 300 s | 300 s → one broker vend per active user every ~5 min |
...
|---|---:|---|
| Lambda: Broker vends (GatewayFn) — 36,000 inv | 0.22 | 36,000 × (1024 MB, 350 ms) |
| Lambda: Admin API (GatewayFn) — 2,000 inv | 0.01 | 2,000 × (1024 MB, 250 ms) |
...
| Line item | USD / month | Basis |
|---|---:|---|
| Lambda: Broker vends (GatewayFn) — 1,440,000 inv | 7.49 | 1,440,000 × (1024 MB, 300 ms) |
| Lambda: Admin API (GatewayFn) — 20,000 inv | 0.09 | 20,000 × (1024 MB, 250 ms) |
...
4. **Use a shorter lease dial with care.** A 60 s lease quintuples broker
   ...
   traffic; the 300 s default is the cost/latency balance the sample ships
   with. 900 s halves vend cost again at the price of a longer post-detection
   bound.
...
6. **Invocation-log ingestion is negligible** with payload delivery disabled
   ...
   generations (README "Priced dimensions") adds the image bytes to
   ingestion; budget for it before turning it on.
