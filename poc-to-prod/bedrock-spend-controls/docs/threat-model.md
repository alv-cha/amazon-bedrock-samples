...
generated from the JSON by `tools/threat_composer_export.py` for reviewers
who work in that tool. `tests/test_threat_model.py` fails when the three
disagree.

Reviewed: 2026-09-15, against the `per-user-quotas` branch after the
multi-dimension pricing, thresholds/rate-limit, model-budget,
reconciliation, and nightly auto-block sweep changes; restructured the same
day into the assets / assumptions / data-flow / grammar form that AWS
security reviewers expect (no threat was added or dropped in that pass).
...
| AS-6 | Overspend is bounded, not zero. The design guarantee is metering lag + min(lease remainder, deny propagation); real-time cut-off is not a goal, and the bound has not yet been measured under load. | T-13, T-16, T-17 |
