...
5. **Credits / private pricing.** CE `UnblendedCost` reflects them; the
   ...
   Seen live (2026-09-15, internal account): six Opus 4.7 calls the ledger
   priced at `$0.00169` billed `$0.000175` (`…-mantle-*` usage types at a
   zero rate), giving `delta_percent ≈ -90 %` — correct behaviour, not a
   bug. An account billed entirely in credits sits at `-100 %` every day;
   raise `reconciliation_alarm_percent` or accept the alarm as the signal
   that the ledger, not the bill, is the number to manage against.
...
7. **Wrong service names.** The Lambda filters CE on `Amazon Bedrock` and
   `Amazon Bedrock Service` (verified live 2026-09-14). If your bill shows
   Bedrock spend under another `SERVICE` value, set `CE_SERVICE_NAMES_JSON`
   on the Lambda and open an issue.

Inspect the runs:
```bash
curl -s -H "Authorization: Bearer $ADMIN_KEY" "$GATEWAY/admin/reconciliation?limit=7" | jq '.runs[] | {day, aggregate, tag_inactive_workloads}'
