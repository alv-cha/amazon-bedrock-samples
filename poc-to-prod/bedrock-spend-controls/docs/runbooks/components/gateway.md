...
| Input | Output |
|---|---|
| `POST /v1/credentials` + JWT in `X-Quota-User-Token` (`Authorization: Bearer` only on local runs without SigV4) | STS keys, `expiration` (lease deadline), `sts_expiration`, `lease_id`, `refresh_after`; quota headers `X-Quota-*` |
