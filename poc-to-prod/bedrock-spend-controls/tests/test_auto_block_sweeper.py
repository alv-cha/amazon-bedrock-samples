...
def test_legacy_row_with_auto_reason_counts_as_automatic(
    fake_dynamodb, fake_sns
):
    ...
    assert result["lifted"] == 1
    row = _row(fake_dynamodb, "legacy-user")
    assert row["status"] == "active"
    assert row["version"] == 1
    assert _row(fake_dynamodb, "REVOCATION#legacy-user")["source_identity"] == ""


def test_legacy_row_without_auto_marker_is_treated_as_manual(
    fake_dynamodb, fake_sns
):
    fake_dynamodb.Table(USERS).put_item(
        Item={
            "user_id": "legacy-user",
            "status": "blocked",
            "status_reason": "blocked by ops during migration",
            "daily_usd_micro": 5_000_000,
            "daily_input_tokens": 0,
            "daily_output_tokens": 0,
        }
    ...
    assert result["admin_blocked"] == 1
    assert _row(fake_dynamodb, "legacy-user")["status"] == "blocked"
