...
# Verified with GetDimensionValues(SearchString="Bedrock") in a live account
# (2026-09-14): on-demand model usage appears under BOTH names depending on
# the SKU. Marketplace-billed third-party models (Anthropic) still land under
# these services in the accounts checked; there is no separate "Claude"
# service value. Override with CE_SERVICE_NAMES_JSON if your bill differs.
...
def _delta(estimated: float, billed: float) -> tuple[float, float | None]:
    ...
    saw nothing the bill charged for (unmetered callers). Dividing by the
    bill alone made tiny bills explode the figure (a $5 ledger against a
    $0.000007 bill read as -72,000,000 %), which is useless as an alarm
    input; the deploy-time ``reconciliation_alarm_percent`` cap of 100
    assumes this bounded scale.
    """
...
def list_results(usage_table, *, limit: int) -> list[dict]:
    """Most recent runs, newest first, from the ``RECONCILE#`` rows."""
    # One row per day keyed RECONCILE#<day>/<day>; a bounded scan on the
    # prefix is simplest and runs only from the admin API.
    response = usage_table.scan(
        FilterExpression=Key("user_id").begins_with(RECONCILE_PREFIX),
    )
